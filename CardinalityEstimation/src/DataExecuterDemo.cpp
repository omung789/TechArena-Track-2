...
void DataExecuterDemo::readTuples(int start, int offset, std::vector<std::vector<int>> &vec) {
    /*
    The readTuples function in the DataExecuterDemo class is designed to read a subset of tuples from the set collection and add them to a provided vector, vec.
    The function takes three parameters: start, offset, and a reference to a std::vector<std::vector<int>> named vec.

    The start parameter specifies the starting index from which to begin reading tuples, while the offset parameter defines the number of tuples to read starting from the start index.
    The vec parameter is a reference to a vector of vectors where the selected tuples will be stored.

    The function enters a for loop that iterates from the start index to start + offset.
    In each iteration, it checks if the current index i has not been visited by examining the vis map at position i.
    The vis map is a std::unordered_map<int, bool> that keeps track of which indices have been visited.

    If the index i has not been visited (!vis[i]), the function adds the tuple at index i from the set collection to the vec vector using the push_back method.
    The set is presumably a member variable of the DataExecuterDemo class, likely a std::vector<std::vector<int>>, which stores all the generated tuples.

    The function continues this process for all indices in the specified range, adding only the unvisited tuples to the vec vector.
    Once the loop completes, the function returns, having populated the vec vector with the selected tuples.
...
double DataExecuterDemo::answer(int ans) {
    /*
    prerequisite C and C++ knowledge:
    - std::fabs() is a function that returns the absolute value of a floating-point number

    The answer function in the DataExecuterDemo class is designed to compute an error metric based on the comparison between an expected answer and the actual count of tuples that meet the conditions.
    The function takes an integer parameter ans, which represents the expected answer.

    The function begins by initializing a counter cnt to zero. This counter will be used to count the number of tuples that satisfy the specified conditions.

    The function then enters a for loop that iterates over all indices from 0 to end (inclusive).
    For each index i, it first checks if the index has been visited by examining the vis map. If vis[i] is true, the function skips the current iteration using the continue statement.

    If the index has not been visited, the function initializes a boolean variable flag to true.
    This flag will be used to determine if the current tuple satisfies all the conditions specified in the curAction.quals vector.

    The function then enters a nested for loop that iterates over all the conditions in the curAction.quals vector. Each condition is represented by a CompareExpression object.
    The function checks if the current tuple at index i satisfies the condition specified by the CompareExpression object.

    If the compareOp of the CompareExpression is GREATER, the function checks if the value in the tuple at the specified column index is less than or equal to the specified value.
    If this condition is met, the flag is set to false, and the loop breaks.

    If the compareOp is EQUAL, the function checks if the value in the tuple at the specified column index is not equal to the specified value.
    If this condition is met, the flag is set to false, and the loop breaks.

    If the flag remains true after checking all conditions, it means that the current tuple satisfies all the conditions, and the counter cnt is incremented by one.

    After the outer loop completes, the function calculates the error metric. The error is computed as the absolute value of the natural logarithm of the ratio of (ans + 1) to (cnt + 1).
    This calculation ensures that the error metric is always positive and provides a measure of the difference between the expected answer and the actual count of tuples that meet the conditions.
